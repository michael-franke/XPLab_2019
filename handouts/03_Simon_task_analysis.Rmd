...
Each trial started by showing a fixation cross for 200 ms in the center of the screen. Then, one of two geometrical shapes was shown for 500 ms. The **target shape** was either a blue square or a blue circle. The target shape appeared either to the left or right. Each trial determined uniformly at random which shape to show as target and where on the screen to display it. Participants where instructed to press keys `p` or `q` to identify the kind of shape on the screen. The shape-key allocation happened experiment initially, uniformly at random once for each participant and remained constant throughout the experiment. If no key had been pressed 3 seconds after the appearance of the target shape a message to please respond faster was displayed on screen.

## Procedure

Participants were first welcomed and made familiar with the experiment. They were told to optimize both speed and accuracy. They then practiced the task for 20 rounds before starting the main task, which consisted of 100 trials. Finally, the experiment ended with a post-test survey in which participants were asked for their studend IDs, the course they were enrolled in, and where they were also able to leave any optional comments.
...
It is conceivable that some participants did not take the task serious. They may have just fooled around. We will therefore inspect each individual's response patterns and reaction times. If participants appear to have "misbehaved" we discard all of their data. (**CAVEAT:** Notice the researcher degrees of freedom in the decision of what counts as "misbehavior"! It is therefore that choices like these are best committed to in advance!)

We can calculate the mean reaction times and the 
...
It is also conceivable that inidividual trials resulted in early accidental key presses or were interrupted in some way or another. We therefore look at the overall distribution of RTs and determine (similarly arbitrarily) what to exclude.
...
Here's a plot of the reaction times split up by whether the answer was correct and whether the trial was congruous or not. 

```{r}
d %>% ggplot(aes(x = RT)) +
  geom_jitter(aes(x = RT, y = 0.0005), alpha = 0.1, height = 0.0005) +
